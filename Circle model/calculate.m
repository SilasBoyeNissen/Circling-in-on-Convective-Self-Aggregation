function R = calculate(G, p, RminL, RmaxL) % outputs [dr x y] for all circles belonging to the subsequent generation
g = find(p(:, 4) == G);
pg = p(g, :);
ID = cellfun(@(v) v(2:end), rangesearch(pg(:, 1:2), pg(1:4:end, 1:2), 2*RmaxL), 'UniformOutput', false);
...
for i = 1:numel(ia)
    ...
    dr(dr < eps) = NaN;
    [dr, j] = min(dr);
    if (dr+r1 > RminL) + (dr+r2 > RminL) + (dr+r1 < RmaxL) + (dr+r2 < RmaxL) == 4
